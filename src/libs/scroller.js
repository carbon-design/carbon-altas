...
class Scroller {
  ...
  init(options) {
    ...
    let component = (this._component = tempContainer.querySelector(
      "[data-role=component]"
    ));
    let content = (this._content = component.querySelector(
      "[data-role=content]"
    ));
    ...
    if (data.length && data[0].constructor === Object) {
      data.forEach(row => {
        html += `<div class="${this.options.itemClass}" data-value="${
          row.value
        }">${row.name}</div>`;
      });
    } else {
    ...
    this._itemHeight = parseInt(
      this._getComputedStyle(indicator, "height"),
      10
    );
  ...
  _setDimensions(ch, cnh) {
    ...
    this._maxScrollTop =
      this._minScrollTop + totalItemCount * this._itemHeight - 0.1;
  }
  ...
  _publish(top, animationDuration) {
    ...
    if (animationDuration) {
      ...
      this._isAnimating = Animate.start(
        ...
        completed,
        animationDuration,
        wasAnimating ? this._easeOutCubic : this._easeInOutCubic
      );
    } else {
  ...
  _scrollingComplete() {
    let index = Math.round(
      (this._scrollTop - this._minScrollTop - this._itemHeight / 2) /
        this._itemHeight
    );
  ...
  _startDeceleration() {
    ...
    const completed = () => {
      this._isDecelerating = false;
      if (
        this._scrollTop <= this._minScrollTop ||
  ...
  _doTouchMove(touches, timeStamp, scale) {
    ...
    if (this._isDragging) {
      ...
      this._isDragging =
        this._enableScrollY && distanceY >= minimumTrackingForDrag;
  ...
  _doTouchEnd(timeStamp) {
    ...
    if (this._isDragging) {
      ...
      if (this._isSingleTouch && timeStamp - this._lastTouchMove <= 100) {
        ...
        for (
          let i = endPos;
          i > 0 && positions[i] > this._lastTouchMove - 100;
          i -= 2
        ) {
          startPos = i;
        }
        ...
        if (startPos !== endPos) {
          ...
          if (
            Math.abs(this._decelerationVelocityY) >
