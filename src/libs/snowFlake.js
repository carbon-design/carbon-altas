...
class SnowFlake {
  ...
  _initAll() {
    ...
    this.progress = 0;
    this.gap =
      (this.options.maxHeight / Math.max(~~Math.random() * 6, 4)) *
      (Math.random() > 0.5 ? 1 : -1);
    this.fluctuation =
      this.options.fluctuation * this.options.maxWidth * Math.random();
    this.size = Math.max(
      this.options.maxSize * Math.random(),
      this.options.minSize
    );
    this.speed = Math.max(
      (this.size / this.options.maxSize) * this.options.maxSpeed,
