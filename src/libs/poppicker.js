...
class PopPicker {
  constructor(options) {
    ...
    if (
      Object.prototype.toString.call(this.options.data[0]) === "[object Object]"
    ) {
    ...
    if (!this.options.defaultValues) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (defVal) {
          ...
          this._values[i] = this._getChildsByParent(
            this._values[i - 1].value,
            data[i]
          )[0];
        }
      }
  ...
  _getFullValues(vals, colIndex) {
    ...
    if (nextIndex < this._scrollSize) {
      let nextGroup = (this._selectGroup[nextIndex] = this._getChildsByParent(
        vals.value,
        data[nextIndex]
      ));
  ...
  _installScroller() {
    this._scrollers = [];
    this._scrollerColums.forEach(item => {
