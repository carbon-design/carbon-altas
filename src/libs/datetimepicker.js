...
class DatetimePicker {
  ...
  _setDayScroller(year, month, day) {
    ...
    this.dayScroller = this._renderScroller(
      div,
      this._makeData("day", year, month),
      day,
      currentValue => {
  ...
  show(value) {
    ...
    let valueMap = (this.valueMap = this._parseDate(
      opts.format,
      value || opts.value
    ));
    let newValueMap = {};
    ...
    if (this._container) {
      ...
      this._each(this.TYPE_MAP, type => {
        this[type + "Scroller"] &&
          this[type + "Scroller"].select(
            this._trimZero(newValueMap[type]),
            false
          );
      });
    } else {
      ...
      this._each(this.TYPE_MAP, type => {
        ...
        if (type === "day") {
          data = this._makeData(
            type,
            this._trimZero(newValueMap.year),
            this._trimZero(newValueMap.month)
          );
        } else {
          data = this._makeData(type);
        }

        this[type + "Scroller"] = this._renderScroller(
          div,
          data,
          this._trimZero(newValueMap[type]),
          currentValue => {
            ...
            if (type === "year") {
              ...
              let currentYear = this.yearScroller
                ? this.yearScroller.value
                : opts.currentYear;
              let currentDay = this.dayScroller.value;
